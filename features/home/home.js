// features/home/home.js - منطق وبرمجة صفحة home.html
...
// --- Mobile Pages Bottom Sheet Logic ---
function openPagesSheet() {
    const backdrop = document.getElementById('pages-sheet-backdrop');
    if (backdrop) {
...
function closePagesSheet(e) {
    if (e && e.target && e.target.closest && e.target.closest('.pages-sheet') && !e.target.closest('.sheet-close-btn') && !e.target.closest('.sheet-grid-item')) {
        return;
    }
    const backdrop = document.getElementById('pages-sheet-backdrop');
    if (backdrop) {
...
window.openPagesSheet = openPagesSheet;
window.closePagesSheet = closePagesSheet;
window.togglePagesSheet = togglePagesSheet;

document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') closePagesSheet();
});

// Swipe down to close on mobile
let touchStartY = 0;
let touchEndY = 0;
const sheetEl = document.getElementById('pages-sheet');
if (sheetEl) {
